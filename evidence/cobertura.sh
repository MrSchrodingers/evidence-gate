...
orchestration/render.py|nao avaliado nesta tarefa: o piso de cobertura de decisao (ADR 0028) fechou a folga do mecanismo ja existente; promover um executavel novo a ALVO exige medir o piso alcancavel dele, investigacao propria, fora do escopo desta correcao pontual. ATUALIZADO na onda 27 (G105/issue #49): render.py deixou de ser so verificador - passou a ESCREVER as vinte projecoes de agente (.claude/agents/*.md, .codex/agents/*.toml), a mesma classe de mudanca que a nota de evidence/probes/ledger-atribuicao.py acima descreve como gatilho de promocao ("se algum ramo dele passar a alimentar decisao, vira ALVO com piso medido"). Continua EXCLUIDO aqui - promover exige medir o piso alcancavel do gerador, investigacao propria, fora do escopo desta correcao pontual - mas a isencao deixou de significar "nao decide nada": passa a significar "decide, e o piso ainda nao foi medido". Manter a redacao antiga sem esta atualizacao, depois de dar poder de escrita ao arquivo, seria a mesma classe de defeito que G105 corrigiu.
